...
export default function StaffPage2() {
  ...
  const [groupBy, setGroupBy] = useState(() => localStorage.getItem('staff2_group_by') || 'position');
  const setGroup = (g) => { setGroupBy(g); localStorage.setItem('staff2_group_by', g); };
  const [view, setView] = useState(() => localStorage.getItem('staff2_view') || 'list');
  const setViewMode = (v) => { setView(v); localStorage.setItem('staff2_view', v); };
